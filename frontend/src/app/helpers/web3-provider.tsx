...
// Move wagmiConfig outside of the Web3Provider component and export it
export let wagmiConfig: ReturnType<typeof createConfig>;

export const createWagmiConfig = (
  walletConnectProjectId: string,
  contractConfig: { chains: ChainConfig[] }
) => {
  const supportedChains = contractConfig.chains.map((chain) => chain.id);
  const chains = supportedChains.map((chainId) => getChain(chainId));
  const transports = generateTransports(contractConfig.chains);
...
// Initialize wagmiConfig in Web3Provider
export const Web3Provider = ({
  children,
  walletConnectProjectId,
}: {
  ...
  createWagmiConfig(walletConnectProjectId, contractConfig);
